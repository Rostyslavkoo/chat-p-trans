...
- **Manager side** (`apps/admin`) — one Next.js app serving **two roles**, split by URL prefix:
  - **`admin`** (`/admin/*`) — the platform operator. Registers sites (which mints the `data-site-id` and its embed snippet) and creates/removes manager accounts, each bound to exactly one site. Has **no access to conversations** at all.
  - **`manager`** (`/chats`, `/ratings`, `/managers`) — support staff for **one** site. Everything they see is scoped to their own `siteId`: conversations, ratings, and colleagues.

  A logged-in user only ever sees their own role's side and their own site's data — the backend scopes by the authenticated session, not by anything the frontend passes. There is no per-tenant subdomain or per-tenant deployment.
...
4. **Two roles (`admin` / `manager`), scoped by identity rather than URL.** The role decides *which side of the app* you land on (`/admin/*` vs the manager pages) and the session's `siteId` decides *which data* you see — never a subdomain, never a `?tenantId=` the frontend passes around. A manager belongs to exactly one site (`Manager.siteId`); admins have `siteId: null` and no conversation access. If a task seems to need the frontend to specify "which site", that's a sign the design is wrong, not that a new prop/param is needed. Frontend scoping (`useCurrentSiteId`) is a UI convenience — the backend must enforce the same rules independently.
...
- ✅ `apps/admin` — admin side (`/admin/sites`, `/admin/managers`) and manager side (chat inbox, `/ratings`, `/managers`) both built against Zustand-held mock data. See `docs/implementation-plan.md` for what's done vs. pending.
...
- **Multi-tenancy is identity-scoped, not URL-scoped**: one domain, one deployment, no per-tenant subdomain and no `?tenantId=` the frontend passes around. The session decides everything: `user.role` picks the side of the app (`AuthGuard role="admin" | "manager"`, which also drives `Sidebar`'s nav), and `user.siteId` scopes the data. Read the site via `useCurrentSiteId()` rather than reaching into the session store ad hoc — it returns `""` for admins/signed-out users, which filters to an empty list instead of leaking another site's data.
- **Stores hold every site's rows; pages filter.** `conversations.store` / `managers.store` / `sites.store` are seeded with all mock rows the way a backend would return them; scoping to `siteId` happens where they're read (see `(protected)/chats/page.tsx`). Don't bake the current site into a store's state — it would break the moment a real API is swapped in.
